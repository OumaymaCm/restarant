...
<div class="container w-full px-5 py-6 mx-auto flex-grow">
    <div class="grid lg:grid-cols-4 gap-y-6">
        @foreach ($menus as $menu)
        <div class="max-w-xs mx-4 mb-2 rounded-lg shadow-lg">
            <img class="w-full h-48" src="{{ Storage::url($menu->image) }}" alt="Image" />
            <div class="px-6 py-4">
                <h4 class="mb-3 text-xl font-semibold tracking-tight text-black-600 uppercase">{{ ($menu->name) }}</h4>
                <p class="leading-normal text-gray-700">{{ ($menu->description) }}</p>
            </div>
            <div class="flex items-center justify-between p-4">
                <span class="text-xl text-black-600">{{ ($menu->price) }} DH</span>
            </div>
        </div>
        @endforeach
    </div>
...
<footer class="bg-gray-50 border-t border-gray-200">
    <div class="container flex flex-wrap items-center justify-center px-4 py-8 mx-auto lg:justify-between">
        <div class="flex flex-wrap justify-center sm:rounded">
            <p class="text-yellow-700">
                <i class="fas fa-map-marker-alt mr-2"></i> Maka Street, Laayoune, Morocco
            </p>
        </div>
        <div class="flex justify-center mt-4 lg:mt-0 sm:rounded">
